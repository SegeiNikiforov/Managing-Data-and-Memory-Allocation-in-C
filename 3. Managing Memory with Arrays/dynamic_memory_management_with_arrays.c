...
int main(int argc, char *argv[]) {
    ...
    double *hear_distribution_metrics = (double*)malloc(array_size * sizeof(double)),
                     *grinder_metrics = (double*)malloc(array_size * sizeof(double)),
                        *pour_metrics = (double*)malloc(array_size * sizeof(double));

    if (!hear_distribution_metrics)
        hear_distribution_metrics = (double*)malloc(DEFAULT_SIZE * sizeof(double));
    ...
    for(int i = 0; i < 3; i++) {
        printf("Address of the row: %p\n", metrics_matrix[i]);
        for(int j = 0; j < array_size; j++) {
            printf("Row/Col: %d/%d ==> Value: %f %p\n", i + 1, j + 1, metrics_matrix[i][j], &metrics_matrix[i][j]);
        }
    }
...
void gather_metrics(size_t size, double *metric_arr) {
    for(int i = 0; i < size; i++) {
        metric_arr[i] = get_random_number(); // Array notation
//        *(metric_arr + i) = get_random_number(); // Pointer notation
    }
}
